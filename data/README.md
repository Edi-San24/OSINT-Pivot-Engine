...
`training_data_domains_v2.csv` holds 383 labeled domains, 193 malicious and 190 benign.
...
Training is deterministic, with `random_state=42` throughout, so retraining from
...
**Malicious.** Recent domains from ThreatFox and URLhaus, both free and same-day.
...
of one number, with `malicious_votes` and `malicious_ratio` correlating at 0.999,
and any one of them alone scores AUC 0.96, so a model that keeps them mostly
